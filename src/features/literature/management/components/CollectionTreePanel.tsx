...
export const CollectionTreePanel: React.FC<CollectionTreePanelProps> = ({ className, onSelectCollection }) => {
    ...
    const handleCreate = async (options: { parentId?: string | null; type?: Collection['type'] } = {}) => {
        ...
        return (
            <DropdownMenu key={c.id} open={contextOpenId === c.id} onOpenChange={(open) => { if (!open && contextOpenId === c.id) setContextOpenId(null); }}>
                ...
                <DropdownMenuContent align="start" side="right">
                    <DropdownMenuItem onClick={() => { setContextOpenId(null); handleCreate({ parentId: c.id, type: c.type }); }}>
                        新建子集合
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={async () => {
                        setContextOpenId(null);
                        const currentName = c.name;
                        const name = window.prompt('重命名集合', currentName);
                        if (name && name.trim() && name.trim() !== currentName) {
                            try { await updateCollection(c.id, { name: name.trim() } as any); } catch { }
                        }
                    }}>
                        重命名
                    </DropdownMenuItem>
    ...
    return (
        <Card className={className}>
            ...
            <CardContent className="pt-0">
                <div className="text-xs text-muted-foreground mb-2">共 {stats.total} 个集合</div>
                <div className="max-h-[28rem] overflow-auto pr-1">
                    ...
                    <div
                        className={cn(
                            'mt-1 flex items-center gap-2 px-2 py-1 rounded-md cursor-pointer hover:bg-muted',
                        ...
                        return (
                            <div className="mt-2">
                                ...
                                >
                                    ...
                                    <Button size="sm" variant="ghost" className="h-7 px-2" onClick={(e) => { e.stopPropagation(); handleCreate({}); }}>
